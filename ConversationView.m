...
-(void) addTimeStamp
{
	...
	NSLog(@"Stamp Difference: %f", [last_stamp timeIntervalSinceNow]);	
...
- (BOOL)appendToConversation:(NSString *)text fromUser:(Buddy *)user
{
	...
	NSLog(@"Before removing '%@'", m_text);
	[self removeHTML:m_text];
	NSLog(@"After Removing '%@'", m_text);
	...
	NSLog(@"Chat Size (%f, %f)", widest_text, highest_text);
	...
	NSLog(@"(%f, %f) -> (%f, %f)", bubble_area_width, bubble_area_height, bubble_center_width, bubble_center_height);
	...
	for(i; i<[lines count]; i++)
	{
		...
		if(user == nil)
		{
			NSLog(@"Shifting %f->%f", text_x, text_x + (widest_text - [l textSize].width));
