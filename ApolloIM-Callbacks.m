...
static void buddy_event(PurpleBuddy *buddy, PurpleBuddyEvent event)
{
  NSLog(@"BUDDY EVENT ");
	...
	if (buddy) 
	{
		...
		{
			[b setAlias:[NSString stringWithUTF8String:purple_buddy_get_alias(buddy)]];
...
{		
	...
	@try{		
		...
		[[ApolloCore sharedInstance]receivedMessage:[NSString stringWithUTF8String:message] withBuddyName:[NSString stringWithCString:who] fromAcct:purple_conversation_get_account(conv)];
	}
