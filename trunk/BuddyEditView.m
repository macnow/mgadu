...
-(id) initWithFrame:(CGRect) aframe withAccount:(PurpleAccount *) pa
{
	if ((self == [super initWithFrame: aframe]) != nil) 
	{
		...
		[[ggnumber_cell textField] setPreferredKeyboardType: 2];
                ...
                [alias_cell setTitle: [NSString stringWithUTF8String: "Pseudo"]];
...
- (void) buttonEvent:(UIPushButton *)button 
{
	...
	if (![button isPressed] && [button isHighlighted])
		...
		if(button == save_button)
		{
			...
			[self addNewBuddy];
			[[ViewController sharedInstance] showMessage: [NSString stringWithUTF8String: "" ] withTitle:[NSString stringWithUTF8String: "Dodałem nowy kontakt!"]];
			[[ViewController sharedInstance] transitionToBuddyListView];
		}
	}
}
