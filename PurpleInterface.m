...
-(id) init
{
	if ((self == [super init]) != nil) 
	{
		...
		[password_cell setTitle: [NSString stringWithUTF8String: "Hasło"]];
		[[password_cell textField]setText:@""];
		[password_cell setEnabled:YES];
		[password_cell setPlaceHolderValue: [NSString stringWithUTF8String: "Twoje hasło"]];
		...
		[server_cell setTitle:@"Serwer"];
		[server_cell setPlaceHolderValue:@"Inny serwer"];
		...
		[port_cell setPlaceHolderValue:@"Inny port"];
		port_field = [port_cell textField];

		delete_button = [[UIPreferencesTableCell alloc] init];
		[delete_button setTitle: [NSString stringWithUTF8String: "Usuń Konto"]];
...
-(void) logIn:(User *) auser
{
	if([auser isActive])
	{
		...
		{
			if([[auser getStartingName] rangeOfString:@"@"].location == NSNotFound)
...
-(BOOL) supportsService:(NSString *) service
{
	return ([service isEqualToString:AIM] ||
		...
		[service isEqualToString:GG]
		);
}

-(void) fireEvent:(Event *) event
{
	NSLog(@"FIRE Firing Message to %i listeners", [listeners count]);
	int i = 0;
...
-(UIView *) getNewPreferencesViewWithFrame:(CGRect) frame andProtocol:(NSString *) aprotocol;
{
	...
	if([aprotocol isEqualToString:GG])
	{
		[[username_cell textField] setPreferredKeyboardType: 2];
	}
	else
	{
		[[username_cell textField] setPreferredKeyboardType: 0];
	}
